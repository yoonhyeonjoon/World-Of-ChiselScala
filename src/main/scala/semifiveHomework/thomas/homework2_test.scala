...
//import chiselExample.problems.{VolumeIntegrator, VolumeIntegratorParams}
...
class VolumeIntegratorTestabler extends Module{
  ...
  val waves: Seq[Seq[Int]] = fakeWaves(10, 150, 50, 3)

  val testableBundle:VolumeIntegratorTestableBundle = IO(new VolumeIntegratorTestableBundle())
  val duration = 15
//  val volumeIntegrator: VolumeIntegrator = Module(new VolumeIntegrator(VolumeIntegratorParams(maxWaveHeight = 150, inputNumber = 10, timemainFactor = duration)))
  val volumeIntegrator: topIntegration = Module(new topIntegration(envSetting(wave_MaxValue = 150, input_EA = 10, clock_EA = duration)))
  val transmit: Seq[ElementTransmitModule[UInt]] = fakeWaves(10, 200, 50, duration).map(aWaveSet =>
...
object myTest25 extends App{
  println("Starting generate")
  (new ChiselStage).emitVerilog(new VolumeIntegratorTestabler()) //height max value, input EA, clock EA
}